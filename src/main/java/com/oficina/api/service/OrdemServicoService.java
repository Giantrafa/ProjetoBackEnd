...
public class OrdemServicoService {
    ...
    public Page<OrdemServicoResponseDTO> listar(String busca, StatusOrdemServico status, Pageable pageable) {
        return ordemServicoRepository.buscar(busca == null ? null : busca.trim(), status, pageable).map(OrdemServicoResponseDTO::from);
    }
    ...
    public void excluir(Long id) {
        if (!ordemServicoRepository.existsById(id)) {
            throw new RecursoNaoEncontradoException("Ordem de serviço não encontrada com id: " + id);
        }
    ...
    private void preencher(OrdemServicoModel ordem, OrdemServicoRequestDTO request) {
        ...
        if (!veiculo.getCliente().getId().equals(cliente.getId())) {
            throw new RegraDeNegocioException("O veículo selecionado não pertence ao cliente informado.");
        }

        List<ServicoModel> servicos = buscarServicos(request.servicoIds());
        List<PecaModel> pecas = buscarPecas(request.pecaIds());
        TipoCalculoPreco tipo = request.tipoCalculoPreco() == null ? TipoCalculoPreco.PADRAO : request.tipoCalculoPreco();
    ...
    private List<PecaModel> buscarPecas(List<Long> ids) {
        ...
        if (encontradas.size() != ids.size()) {
            throw new RecursoNaoEncontradoException("Uma ou mais peças informadas não foram encontradas.");
        }
        return encontradas;
    }
